...
```{r}
lm <- lm(freehms ~ PVQ*n_de_mu_im_pop + cntry, data = samlet_ess)
lm_rob <- lmrob(freehms ~ PVQ*n_de_mu_im_pop + cntry, data = samlet_ess, control = lmrob.control(k.max = 500, fast.s.large.n = Inf))
stargazer(lm, lm_rob, type = "text")
```

```{r}
df <- samlet_ess %>%
  filter(!is.na(freehms) & !is.na(n_de_mu_im_pop) & !is.na(PVQ) & !is.na(cntry))%>%
  dplyr::select(c("PVQ", "freehms", "n_de_mu_im_pop", "cntry"))
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/residualer.png", width = 7.5, height = 4, units = "in", res=500)
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/plotr.png", width = 7.5, height = 7.5, units = "in", res=500)
par(mfrow = c(2, 2))
plot(lm)
dev.off()
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/homoskedadastitet.png", width = 7.5, height = 7.5, units = "in", res=500)
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/normal fordeling.png", width = 7.5, height = 7.5, units = "in", res=500)
...
r
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/Multicollinearity.png", width = 7.5, height = 5, units = "in", res=500)
plot(r)
dev.off()
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/outliers.png", width = 7.5, height = 5, units = "in", res=500)
check_outliers(lm)
...
```{r}
png(file="../../figures/forudsætningstest 1 år/model 1 uden kontrol/check model.png", width = 7.5, height = 9, units = "in", res=500)
